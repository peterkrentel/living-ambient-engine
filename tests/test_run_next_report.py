...
def test_build_markdown_lists_actionable_and_indices():
    ...
    md = rn.build_markdown(
        ...
        audit,
        personal_line="`data/analytics_personal.json` — **3** videos",
        latest_personal_report="data/reports/2026-W01-personal.md",
    )
    assert "Packaging & confounders" in md
    assert "`suggestions.json` → `suggestions[1]`" in md
