...
total_watch = sum(v['metrics']['watch_time_minutes'] for v in videos)
...
for vtype, vids in by_type.items():
    views = sum(v['metrics']['views'] for v in vids)
    with_data = [v for v in vids if v['metrics']['views'] > 0]
    if with_data:
        avg_ret = sum(v['metrics']['average_view_percentage'] for v in with_data) / len(with_data)
        avg_dur = sum(v['metrics']['average_view_duration_seconds'] for v in with_data) / len(with_data)
    else:
        avg_ret, avg_dur = 0, 0
...
ranked = sorted(with_views, key=lambda v: v['metrics']['average_view_percentage'], reverse=True)[:5]
