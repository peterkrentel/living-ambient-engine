...
**Advisory `run-next`:** **Shipped (v0):** weekly markdown **`data/reports/run-next-YYYY-WW.md`** (brand) and **`data/reports/run-next-YYYY-WW-personal.md`** (personal) from correlate JSON + channel audit — **actionable** vs **exploratory**, packaging caveat ([`scripts/run_next_report.py`](../scripts/run_next_report.py); brand [`analytics-agent.yml`](../.github/workflows/analytics-agent.yml), personal [`analytics-personal.yml`](../.github/workflows/analytics-personal.yml)). **Separate** from the **strict** automation gate (`data/run_intent.json` / `run-intent-blocked.md`): humans read `run-next` for strategy; they run Content Factory / consumer only when ready. **Packaging / algorithm surface:** same-franchise + slight title/thumb variation — **[`piano-batch.yml`](../.github/workflows/piano-batch.yml)** as repo example (personal cross-read). **Confounders:** [`AGENT.md`](spec/AGENT.md) § *Confounders & packaging*; per-channel **dedup / positioning** later ([`HANDOFF.md`](HANDOFF.md) next actions **#8**).

**Execution variety (incremental):** **Phase 1 shipped** — [`scripts/plan_run_intent.py`](../scripts/plan_run_intent.py) **ledger anti-repeat** (`RUN_INTENT_ANTI_REPEAT_WEEKS` in both analytics workflows): skip **mood × duration** recently uploaded on the **same** channel using `data/generations.json`. **Next slices:** deterministic **packaging angle** matrix (same gen params, different title promise) → optional **constrained LLM** title candidates fed into upload — new fields or sidecar JSON **before** `batch_generate`, contract-validated; see [`production-run-intent.md`](spec/contracts/production-run-intent.md) + [`workflows.md`](spec/workflows.md) § Analytics Agent.
