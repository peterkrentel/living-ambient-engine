...
| Open | **`run-next` follow-ups (tranche 2):** deeper checks (**cited `suggestions[i]`** / audit excerpts vs JSON); optional **LLM prose** on fixed bundle (**self-hosted / open-weight** preferred; **Gemini** prototype only); **auto-chain** consumer only when trusted. [`COHESION_ROADMAP.md`](COHESION_ROADMAP.md) Phase 6 · [`HANDOFF.md`](HANDOFF.md) **#9**. |
...
| Open | **Autonomy horizon (defer):** validated **structured** plan (intent contract + validators) → generate → publish → ledger loop; Phase 3 predictive modeling when justified — not a fixed timeline. [`COHESION_ROADMAP.md`](COHESION_ROADMAP.md) Phase 6 *Autonomy horizon* · [`HANDOFF.md`](HANDOFF.md) **#10**. |
