...
| Open | **Advisory “run-next”:** phased: **(1)** v0 **deterministic** file from `suggestions.json` + audit + two-channel read in [`analytics-agent.yml`](../.github/workflows/analytics-agent.yml) (**actionable** vs **exploratory**, packaging caveat; e.g. [`piano-batch.yml`](../.github/workflows/piano-batch.yml) strategy line). **(2)** **Validators** (cited ids, numbers match JSON). **(3)** Optional **LLM prose** on fixed bundle — **self-hosted / open-weight** preferred; **Gemini** only as short-lived prototype if needed. **(4)** Auto-chain to consumer later. **No** batch/upload from this artifact. [`COHESION_ROADMAP.md`](COHESION_ROADMAP.md) Phase 6 (`run-next` implementation order) · [`HANDOFF.md`](HANDOFF.md) **#9**. |
