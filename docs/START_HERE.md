...
| Open | **Dual advisory refinement:** runner contract is now **CI-safe** (schema/grounding/echo guardrails). **Shipped:** analytics workflows run [`scripts/validate_agent_insight_runner.py`](../scripts/validate_agent_insight_runner.py) after `agent_dual_advisory.py` — channel totals in `*-runner.md` must match sums from `analytics.json` / `analytics_personal.json` (skip if GGUF not run). Remaining work: remove runner placeholder residue (WIR bullets), enforce minimum Insight count / concreteness, optional Gemini checks. Human compare stays manual via [`spec/DUAL_ADVISORY_COMPARE.md`](spec/DUAL_ADVISORY_COMPARE.md). [`COHESION_ROADMAP.md`](COHESION_ROADMAP.md) Phase 6 · [`HANDOFF.md`](HANDOFF.md). |
