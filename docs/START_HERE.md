...
| Open | **Dual advisory refinement:** **human** compare **`agent-insight-*-gemini.md`** vs **`*-runner.md`** vs **`run-next`** / committed JSON (same ISO week + lane); no in-repo diff, merge, or scorer. **How:** read-order + rubric + trust rules in [`spec/DUAL_ADVISORY_COMPARE.md`](spec/DUAL_ADVISORY_COMPARE.md). **Next:** week-over-week check that runner prose matches **deterministic facts**; **validators** (numeric parity) before any automation consumes prose; optional **3B** GGUF (`AGENT_GGUF_URL` + cache key); drop Gemini if marginal value is low. [`COHESION_ROADMAP.md`](COHESION_ROADMAP.md) Phase 6 · [`HANDOFF.md`](HANDOFF.md) **#9**. |
