...
| Open | **Dual advisory refinement:** runner contract is now **CI-safe** (schema/grounding/echo guardrails). **Shipped:** [`validate_agent_insight_runner.py`](../scripts/validate_agent_insight_runner.py) after `agent_dual_advisory.py` — channel totals vs `analytics*.json`; **WIR** rubric-echo bullets stripped at write time + CI rejects placeholders / requires ≥2 **Insights** (flexible list shapes). **`agent-insight-*` markdown does not trigger** [`run-intent-consumer.yml`](../.github/workflows/run-intent-consumer.yml) — see [`COHESION_ROADMAP.md`](COHESION_ROADMAP.md) Phase 6 **LLM advisory → production (contract bridge)**. Remaining: runner completion reliability (`finish_reason=length`); stronger concreteness / Gemini-side checks. Human compare: [`spec/DUAL_ADVISORY_COMPARE.md`](spec/DUAL_ADVISORY_COMPARE.md). [`COHESION_ROADMAP.md`](COHESION_ROADMAP.md) Phase 6 · [`HANDOFF.md`](HANDOFF.md). |
