...
| Open | **Advisory “run-next”:** weekly ranked **what next** from analytics + audit + two-channel context (**actionable** vs **exploratory**); include **packaging / title-variation** notes where relevant (platforms often favor *similar premise + slight metadata variation*—e.g. [`piano-batch.yml`](../.github/workflows/piano-batch.yml) after cross-reading **personal**). **No** automation—artifact only; see [`COHESION_ROADMAP.md`](COHESION_ROADMAP.md) Phase 6 step 1 · [`HANDOFF.md`](HANDOFF.md) **#9**. |
