...
| Open | **Dual advisory refinement:** runner contract is now **CI-safe** (schema/grounding/echo guardrails). Remaining work is **quality + validators**: remove runner placeholder residue (WIR bullets), enforce minimum Insight count / concreteness, and add numeric parity validators before anything consumes prose. Human compare stays manual via [`spec/DUAL_ADVISORY_COMPARE.md`](spec/DUAL_ADVISORY_COMPARE.md). [`COHESION_ROADMAP.md`](COHESION_ROADMAP.md) Phase 6 · [`HANDOFF.md`](HANDOFF.md). |
