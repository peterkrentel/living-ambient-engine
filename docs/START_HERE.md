...
| Open | **Dual advisory refinement:** **human** compare **`agent-insight-*-gemini.md`** vs **`*-runner.md`** vs **`run-next`** / committed JSON (same ISO week + lane); no in-repo diff, merge, or scorer. **Next:** tighten **inputs** (lane-isolated lean CONTEXT, caps), **outputs** (grounding, optional `temperature`/`seed`), **validators** before any automation consumes prose; drop Gemini if marginal value is low. [`COHESION_ROADMAP.md`](COHESION_ROADMAP.md) Phase 6 · [`HANDOFF.md`](HANDOFF.md) **#9**. |
