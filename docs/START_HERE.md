...
**Two analytics experiments (cross-read intentionally):**

- **Brand** — [`analytics-agent.yml`](../.github/workflows/analytics-agent.yml): **`data/analytics.json`**, weekly **`data/reports/YYYY-WW.md`**, then **`scripts/analyze_data.py`** → **`scripts/correlate.py`** → **`data/suggestions.json`** → **`scripts/plan_run_intent.py`** → **`data/run_intent.json`** or **`data/reports/run-intent-blocked.md`** → **`scripts/audit_channel.py`** → **`data/reports/audit-*.md`** → **`scripts/run_next_report.py`** + **`scripts/validate_run_next.py`** → **`data/reports/run-next-YYYY-WW.md`**, then optional **`scripts/agent_dual_advisory.py`** (Gemini + runner GGUF) → **`data/reports/agent-insight-*-brand-*.md`** + **`scripts/validate_agent_insight_runner.py`**. **YAML order:** correlate → plan intent → **audit** → run-next (planner uses suggestions only; run-next reads audit). Canonical list: [`spec/workflows.md`](spec/workflows.md#analytics-agentyml).

- **Personal** — [`analytics-personal.yml`](../.github/workflows/analytics-personal.yml): same *family* of steps with **`data/analytics_personal.json`**, **`*-personal.md`**, **`data/suggestions_personal.json`**, **`run_intent_personal.json`** / blocked, **`run-next-*-personal.md`**, **`agent-insight-*-personal-*.md`**, and the same validators (**`validate_run_next`**, **`validate_agent_insight_runner`**). **YAML order differs:** **audit → correlate** (then plan intent → run-next). The personal job **never** overwrites brand **`suggestions.json`** or **`run_intent.json`**. See [`PERSONAL_ANALYTICS.md`](PERSONAL_ANALYTICS.md).
...
| Open | **Cohesion readiness review (parked):** [`cohesion-readiness-review-todos.md`](cohesion-readiness-review-todos.md) — workflow/test/governance findings; [execution queue](cohesion-readiness-review-todos.md#execution-queue-do-one-at-a-time) (`CR-1` … `CR-6`). One item per PR when resuming. |
...
| Done (v1 parity) | **`run-next` CI validation:** [`validate_run_next.py`](../scripts/validate_run_next.py) runs after each `run_next_report` write in **both** analytics workflows — headline snapshot numbers + **`generated_at`** vs **`suggestions*.json`**; **`--week`** uses **Python UTC ISO week** (same as `plan_run_intent`). **Merged PR #124.** PR CI also runs [`tests/test_validate_run_next.py`](../tests/test_validate_run_next.py) via [`test-art-creator.yml`](../.github/workflows/test-art-creator.yml). |
| Done (v0 plumbing) | **Dual LLM advisory:** [`analytics-agent.yml`](../.github/workflows/analytics-agent.yml) + [`analytics-personal.yml`](../.github/workflows/analytics-personal.yml) run [`agent_dual_advisory.py`](../scripts/agent_dual_advisory.py) → **`agent-insight-*-gemini.md`** + **`agent-insight-*-runner.md`** (Gemini **full** bundle; runner **lean** bundle with **deterministic facts** JSON + **`coverage_summary`** + views/retention compacts). **Secrets:** brand **`GEMINI_API_KEY`**; personal **`GEMINI_API_KEY_PERSONAL`**. Stub Gemini file if key missing/wrong at job time. |
| Done | **`run-next` tranche 2:** [`validate_run_next.py`](../scripts/validate_run_next.py) — cited **`suggestions[i]`** + audit overview excerpt vs audit file (see `docs/spec/workflows.md` § run-next / run-intent-consumer). |
| Open | **`run-next` / advisory follow-ups:** optional **LLM prose** on fixed bundle (**self-hosted / open-weight** preferred; **Gemini** prototype only). [`COHESION_ROADMAP.md`](COHESION_ROADMAP.md) Phase 6 · [`HANDOFF.md`](HANDOFF.md). |
| Open | **Dual advisory refinement:** runner contract is **CI-safe** (schema/grounding/echo guardrails). **Shipped:** [`validate_agent_insight_runner.py`](../scripts/validate_agent_insight_runner.py) — channel totals vs `analytics*.json`; flexible **Insights** counting (**PR #121**); **WIR** rubric-echo strip at write + CI placeholder ban. **Shipped:** leaner runner CONTEXT + **2–4** Insights prompt for token headroom (**PR #123**). **Docs:** dual MD does **not** trigger consumer — **LLM advisory → production (contract bridge)** (**PR #122**). **Remaining:** watch **`finish_reason=length`** in logs after #123; stronger Gemini grounding checks. Human compare: [`spec/DUAL_ADVISORY_COMPARE.md`](spec/DUAL_ADVISORY_COMPARE.md). [`COHESION_ROADMAP.md`](COHESION_ROADMAP.md) Phase 6 · [`HANDOFF.md`](HANDOFF.md). |
| Done (partial) | **Autonomy — intent consumer:** [`run-intent-consumer.yml`](../.github/workflows/run-intent-consumer.yml) **`workflow_run`** after successful **Analytics Agent** / **Analytics Agent (Personal)** on **`main`** → validate → **`batch_generate`** / **`youtube_upload`** when intent **`upload`: true`** (planner uses **`--upload`** in both analytics workflows). |
| Open | **CR-7 — consumer when planner BLOCKED:** auto path must **skip cleanly** (no `consume_run_intent` failure; no `generate` without outputs) — [`cohesion-readiness-review-todos.md`](cohesion-readiness-review-todos.md). |
| Open | **Autonomy horizon (defer):** richer `run_intent` (e.g. duration from data), Phase 3 predictive modeling when justified — not a fixed timeline. [`COHESION_ROADMAP.md`](COHESION_ROADMAP.md) Phase 6 *Autonomy horizon* · [`HANDOFF.md`](HANDOFF.md). |
...
| Done | **Run intent (planner + consumer):** [`scripts/plan_run_intent.py`](../scripts/plan_run_intent.py) (**`--upload`**) after correlate → `data/run_intent*.json` or `run-intent-blocked*.md`; [`run-intent-consumer.yml`](../.github/workflows/run-intent-consumer.yml) — **`workflow_dispatch`** (manual + **`confirm_upload`**) **and** **`workflow_run`** after analytics on **`main`** ([`spec/contracts/production-run-intent.md`](spec/contracts/production-run-intent.md)). |
| Done (phase 1) | **Planner ledger anti-repeat:** same script consults **`data/generations.json`** when **`RUN_INTENT_ANTI_REPEAT_WEEKS=4`** (set in **both** analytics workflows) and skips **mood × duration** already uploaded on that **channel** in the window; legacy rows without **`channel`** / **`uploaded_at`** ignored. Tests: [`tests/test_plan_run_intent_anti_repeat.py`](../tests/test_plan_run_intent_anti_repeat.py). **Next slices:** angle matrix, constrained LLM titles — [`COHESION_ROADMAP.md`](COHESION_ROADMAP.md) Phase 6. |
...
| Open (defer) | **Packaging variety beyond anti-repeat:** **angle matrix** (deterministic promises), **constrained LLM** titles, sharper **brand vs personal** positioning; mood×duration **ledger anti-repeat** is **done** — see [`HANDOFF.md`](HANDOFF.md) next actions #8. |
