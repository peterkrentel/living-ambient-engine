...
| Open | **Dual advisory comparison:** analytics writes **`agent-insight-*-gemini.md`** and **`agent-insight-*-runner.md`** side by side (same ISO week + lane). **No** in-repo automated diff, merge, or scorer—**you** read both against **`run-next`** / committed JSON; agreement is higher confidence; drop Gemini if marginal value is low. Optional later: **validators** on any LLM prose before automation touches it. [`COHESION_ROADMAP.md`](COHESION_ROADMAP.md) Phase 6 *Dual advisory refinement*. |
