...
10. **`run-next` + dual-advisory (quality phase):** wiring is shipped; **CI** now runs **`validate_agent_insight_runner.py`** after dual advisory (runner markdown must quote **analytics** channel totals or skip when GGUF absent). Next: remove runner WIR placeholder residue, enforce minimum Insight count / concreteness, optional Gemini checks before anything consumes prose.
