...
- **Cohesion readiness review (risk / readiness audit):** full findings, maturity verdict, release checklist, and **ordered execution queue** (`CR-1` … **`CR-7`**) — [`cohesion-readiness-review-todos.md`](cohesion-readiness-review-todos.md). Tackle **one `CR-*` item per PR**; check boxes there when done.

## Anchor

- **Analytics (two lanes):** **Brand** — [`analytics-agent.yml`](../.github/workflows/analytics-agent.yml) → `data/analytics.json`, `data/reports/YYYY-WW.md`, `suggestions.json`, **`plan_run_intent.py --upload`** → `run_intent.json` / `run-intent-blocked.md`, `audit-*.md`, `run-next-*.md` (commit step uses `git pull --rebase` before push). **Personal** — [`analytics-personal.yml`](../.github/workflows/analytics-personal.yml) → `data/analytics_personal.json`, `*-personal.md`, `audit-*-personal.md`, `suggestions_personal.json`, same planner (**`--upload`**) → `run_intent_personal.json` / blocked-personal, `run-next-*-personal.md`. After each successful analytics run on **`main`**, **[`run-intent-consumer.yml`](../.github/workflows/run-intent-consumer.yml)** may **auto** chain (**`workflow_run`**) when a lane intent JSON exists. Map: [`START_HERE`](START_HERE.md#two-channels-two-probes) · detail: [`PERSONAL_ANALYTICS`](PERSONAL_ANALYTICS.md).
...
- **`run-next` v0:** [`scripts/run_next_report.py`](../scripts/run_next_report.py) on **brand + personal** workflows → `run-next-YYYY-WW.md` / `run-next-YYYY-WW-personal.md` (deterministic; **actionable** vs **exploratory**; packaging caveat). **Shipped:** [`validate_run_next.py`](../scripts/validate_run_next.py) **tranche 1 + 2** after each write — snapshot + **`generated_at`**, cited **`suggestions[i]`** rows, audit overview excerpt vs audit `## Overview` (**Python ISO week** for `--week`). **Next (optional):** LLM on fixed bundle, correlate length buckets, planner vN — [`COHESION_ROADMAP.md`](COHESION_ROADMAP.md) Phase 6.
...
- **Autonomous production (shipped):** analytics on **`main`** → **`workflow_run`** → [`run-intent-consumer.yml`](../.github/workflows/run-intent-consumer.yml) validates lane intent → **`batch_generate`** → **`youtube_upload`** when intent **`upload`: true`** (auto path skips **`confirm_upload`**; manual dispatch still double-gated). **Planner `duration`** remains CLI default (**`10min`**) unless **`RUN_INTENT_DURATION`** / **`--duration`** is set — not inferred from correlate.
- **Todo:** when planner **BLOCKED** (no `run_intent*.json`), auto consumer must **no-op green** and skip generate — [`cohesion-readiness-review-todos.md`](cohesion-readiness-review-todos.md) **CR-7**.
...
2. **Personal parity (optional):** personal lane matches brand analytics **family** (incl. auto consumer); remaining: personal-aware **audit/join filtering** on catalog when needed — [`PERSONAL_ANALYTICS.md`](PERSONAL_ANALYTICS.md).
...
4. **Run-intent automation:** **Shipped:** both analytics workflows run **`plan_run_intent.py --upload`**; **[`run-intent-consumer.yml`](../.github/workflows/run-intent-consumer.yml)** **`workflow_run`** after successful **Analytics Agent** / **Analytics Agent (Personal)** on **`main`** (fixed intent paths per lane). **Manual** dispatch unchanged (**`confirm_upload`** + intent **`upload`**). **Next:** **CR-7** — graceful skip when planner **BLOCKED** (no intent file) — [`cohesion-readiness-review-todos.md`](cohesion-readiness-review-todos.md). Further: richer intent→flags mapping, LLM overrides — [`COHESION_ROADMAP.md`](COHESION_ROADMAP.md) Phase 6 *Two doors*.
