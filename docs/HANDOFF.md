...
10. **`run-next` + dual-advisory (quality phase):** **CI** runs **`validate_agent_insight_runner.py`** (totals vs **analytics** + WIR placeholder ban + ≥2 numbered Insights); **`agent_dual_advisory.py`** strips rubric-echo WIR lines before write. Next: stronger concreteness / optional Gemini checks before anything consumes prose.
