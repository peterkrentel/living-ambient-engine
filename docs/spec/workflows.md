...
8. Plan run intent (`scripts/plan_run_intent.py --week "$WEEK" --upload`) — gated v0: writes `data/run_intent.json` when actionable **mood** increases exist, else `data/reports/run-intent-blocked.md`. **Anti-repeat (shipped):** job sets **`RUN_INTENT_ANTI_REPEAT_WEEKS=4`** so moods whose **mood × duration_seconds** match a **brand** row in `data/generations.json` with **`uploaded_at`** in the last 28 days are skipped (rows without `channel` or `uploaded_at` are ignored). **[`run-intent-consumer.yml`](../../.github/workflows/run-intent-consumer.yml)** may run automatically after this workflow succeeds on **`main`** (see § **run-intent-consumer.yml**); see [`contracts/production-run-intent.md`](./contracts/production-run-intent.md)
...
12. **Dual LLM advisory (optional v0):** `pip install llama-cpp-python`, then **`scripts/agent_dual_advisory.py --lane brand`** with the same ISO week as step 10. The script runs **Gemini (REST)** on the **full** bundle and **runner GGUF** on a **lean** bundle (**deterministic facts** JSON first — summed views / watch minutes from `analytics.json` + headline fields from `suggestions.json` — then **run-next digest + tail** (not the full run-next file), compact suggestions with **`coverage_summary`** + compact analytics **views + retention** slices + shorter weekly/blocked excerpts) in **parallel threads**, writing `agent-insight-YYYY-WW-brand-gemini.md` and `agent-insight-YYYY-WW-brand-runner.md`. Job logs: expand the **Dual advisory** group; **`[runner-advisory]`** (GGUF: bundle, `n_ctx`, `temperature`, timing, `finish_reason`); **`[gemini-advisory]`** (REST: `usageMetadata`, `finishReason`, prose size). If `GEMINI_API_KEY` is unset, Gemini output is a short stub; if `llama-cpp-python` is missing, runner output is a stub (CI installs it). **Then** **`scripts/validate_agent_insight_runner.py --lane brand --week …`** checks channel totals vs `analytics.json`, rejects rubric-echo **What I reviewed** placeholders, and requires **≥2** **Insights** items under `## Insights` (numbered lists, `-`/`*` bullets, or `###` subheads); **skips** when the runner file documents GGUF skip. The dual-advisory script also **strips** common placeholder WIR bullets before writing `*-runner.md`.
...
| Run-next (v0) | `data/reports/run-next-YYYY-WW.md` | Advisory “what next” from correlate + brand audit + personal pointers; **CI:** [`validate_run_next.py`](../../scripts/validate_run_next.py) (snapshot + `generated_at`, cited **`suggestions[i]`** rows, audit overview excerpt vs `audit-YYYY-WW.md`) |
...
| Runner GGUF advisory (v0) | `data/reports/agent-insight-YYYY-WW-brand-runner.md` | Instruct GGUF on the workflow runner (default Qwen2.5-1.5B q4 path); lean CONTEXT includes **deterministic facts** JSON + **run-next digest/tail**; stub if `llama-cpp-python` / model missing |
...
**Optional (script / workflow `env`):** `GEMINI_MIN_INTERVAL_SEC` (default `6`), `GEMINI_MAX_RETRIES` (default `5`), `GEMINI_429_MIN_SLEEP_SEC` (default `0`), `GEMINI_MAX_OUTPUT_TOKENS` (default `4096`), `GEMINI_THINKING_BUDGET` (default **`0`** for model ids containing `2.5` — disables [hidden thinking](https://ai.google.dev/gemini-api/docs/thinking) so advisory prose is not starved; use `omit` to skip `thinkingConfig` for older models) — see [`agent_dual_advisory.py`](../../scripts/agent_dual_advisory.py). Brand analytics usually leaves these unset (one request per job). **Personal** workflow sets conservative pacing on the dual-advisory step for tight free-tier quotas (often **~5 RPM** class limits on Flash in AI Studio for small projects).
...
8. Plan run intent (`scripts/plan_run_intent.py` with `--suggestions data/suggestions_personal.json`, `--channel personal`, **`--upload`**, `--intent-output data/run_intent_personal.json`, `--blocked-output data/reports/run-intent-blocked-personal.md`) — same **anti-repeat** behavior as brand with **`RUN_INTENT_ANTI_REPEAT_WEEKS=4`**, scoped to **`channel: personal`** rows in `data/generations.json`. When an intent file is written, **[`run-intent-consumer.yml`](../../.github/workflows/run-intent-consumer.yml)** can also run automatically after this workflow succeeds on **`main`** (generate + upload if intent **`upload`: true**; see § **run-intent-consumer.yml**).
