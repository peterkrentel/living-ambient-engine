...
**Optional env (runner GGUF, workflow or runner config):** `AGENT_GGUF_PATH`, `AGENT_GGUF_URL` (default Hugging Face Qwen2.5-1.5B Instruct q4), `AGENT_LLAMA_THREADS`, `MAX_RUNNER_TOKENS` (default `1536` — raise if markdown stops mid-section with `finish_reason=length` in logs), **`RUNNER_BUNDLE_MAX_CHARS`** (default `5200` — lean CONTEXT cap), **`AGENT_LLAMA_N_CTX`** (default `4096`, clamp 2048–8192), **`AGENT_RUNNER_TEMPERATURE`** (default `0.15`) — all in [`agent_dual_advisory.py`](../../scripts/agent_dual_advisory.py).
