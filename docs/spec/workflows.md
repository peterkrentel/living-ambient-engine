...
10. Write **`run-next`** advisory (`scripts/run_next_report.py --week "$WEEK"`) — deterministic markdown from **`suggestions.json`** + **`audit-YYYY-WW.md`** + optional personal-lane pointers (**no** LLM, **no** `batch_generate` / upload); **`$WEEK`** is **UTC ISO week** from Python `isocalendar()` (same as `plan_run_intent`). **Then** **`scripts/validate_run_next.py`** checks snapshot numbers and **`generated_at`** vs `suggestions.json`.
...
| Run-next (v0) | `data/reports/run-next-YYYY-WW.md` | Advisory “what next” from correlate + brand audit + personal pointers; **CI:** [`validate_run_next.py`](../../scripts/validate_run_next.py) (snapshot + `generated_at` vs `suggestions.json`) |
...
9. Run-next advisory personal (`scripts/run_next_report.py --lane personal --week "$WEEK"`) then **`validate_run_next.py`** (same **`$WEEK`**, `data/suggestions_personal.json`)
