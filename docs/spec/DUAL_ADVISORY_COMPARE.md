...
- **Input:** **Lean** bundle — scope banner, then a **deterministic facts** JSON block (Python-summed `sum_views_all_videos`, `sum_watch_time_minutes_all_videos`, `count_videos_with_views_gt_0` from `analytics*.json` plus headline fields from `suggestions*.json`), then a **trimmed run-next** (snapshot digest + actionable tail; cross-lane block removed from the tail), short weekly + blocked reads, **compact** suggestions (incl. `coverage_summary`) and analytics (top by views + retention slice), hard char cap for small `n_ctx`.
