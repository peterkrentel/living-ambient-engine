...
| Consumer | Role |
|----------|------|
| [`run-intent-consumer.yml`](../../../.github/workflows/run-intent-consumer.yml) | **`workflow_dispatch`:** validate intent (defaults brand paths; **personal manual:** `data/run_intent_personal.json` + `data/reports/run-intent-blocked-personal.md`) → `batch_generate.py` → artifact; **YouTube** if intent **`upload`: true** *and* (**`confirm_upload`** on dispatch *or* **auto path** below). **`workflow_run`:** after successful **Analytics Agent** or **Analytics Agent (Personal)** on **`main`**, validates fixed paths for that lane → generate → upload when intent **`upload`: true** (no **`confirm_upload`**). **`validate_only`:** missing intent + paired blocked report → **green** (planner BLOCKED, not a failure). `--catalog-channel` matches intent `channel`. |
| [`scripts/plan_run_intent.py`](../../../scripts/plan_run_intent.py) | **v0 (shipped):** reads suggestions JSON → writes intent JSON **or** blocked markdown. **Brand:** `data/suggestions.json` → `data/run_intent.json` / `run-intent-blocked.md`; **`--upload`** in `analytics-agent.yml`. **Personal:** `--suggestions data/suggestions_personal.json`, **`--upload`**, `--intent-output data/run_intent_personal.json` / `--blocked-output …-personal.md` (`analytics-personal.yml`). Without **`--upload`**, CLI default keeps **`upload`** false. Optional `--force-moods` for smoke. Wired after correlate in both analytics workflows. **Anti-repeat (phase 1):** optional **`--anti-repeat-weeks N`** / env **`RUN_INTENT_ANTI_REPEAT_WEEKS`** — before emitting intent, drop any mood whose **(mood, duration_seconds)** matches a ledger row for the **same** `channel` with **`uploaded_at`** in the last **N×7** days (`data/generations.json`; rows missing `channel` or `uploaded_at` skipped). Default **N=0** locally; both analytics workflows set **`RUN_INTENT_ANTI_REPEAT_WEEKS=4`**. |
