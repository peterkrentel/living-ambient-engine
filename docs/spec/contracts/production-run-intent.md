...
| Consumer | Role |
|----------|------|
| [`run-intent-consumer.yml`](../../../.github/workflows/run-intent-consumer.yml) | **`workflow_dispatch`:** validate intent (default `data/run_intent.json`; **personal manual:** set inputs to `data/run_intent_personal.json` + `data/reports/run-intent-blocked-personal.md`) → `batch_generate.py` → artifact; **YouTube** if intent **`upload`: true** *and* (**`confirm_upload`** on dispatch *or* **auto path** below). **`workflow_run`:** after successful **Analytics Agent (Personal)** on **`main`**, validates fixed personal paths → generate → upload when intent **`upload`: true** (no **`confirm_upload`**). **`validate_only`:** missing intent + paired blocked report → **green** (planner BLOCKED, not a failure). `--catalog-channel` matches intent `channel`. |
| [`scripts/plan_run_intent.py`](../../../scripts/plan_run_intent.py) | **v0 (shipped):** reads suggestions JSON → writes intent JSON **or** blocked markdown; **brand** defaults (`data/suggestions.json` → `data/run_intent.json` / `run-intent-blocked.md`); **`upload`** defaults **false** (no **`--upload`** in `analytics-agent.yml`). **Personal:** `--suggestions data/suggestions_personal.json`, **`--upload`**, `--intent-output data/run_intent_personal.json` / `--blocked-output …-personal.md` (see `analytics-personal.yml`). Optional `--force-moods` for smoke. Wired from `analytics-agent.yml` and `analytics-personal.yml` after correlate. |
