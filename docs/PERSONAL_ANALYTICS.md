...
1. **Operate:** run **`analytics-personal.yml`** on `main` (`workflow_dispatch` first is fine); confirm `data/analytics_personal.json`, `data/reports/*-personal.md`, **`data/suggestions_personal.json`**, **`data/run_intent_personal.json`** (or **`run-intent-blocked-personal.md`**), and **`data/reports/run-next-*-personal.md`** commit cleanly and **never** touch `data/analytics.json` / `data/suggestions.json` / `data/run_intent.json`.
2. **Parity — correlate + plan intent + run-next:** **Shipped** in CI: **`scripts/correlate.py`** with `SUGGESTIONS_JSON_PATH` → **`suggestions_personal.json`**, **`scripts/plan_run_intent.py`** (**`--upload`**) → **`run_intent_personal.json`** / **`run-intent-blocked-personal.md`**, **`scripts/run_next_report.py --lane personal`**. After a successful analytics run on **`main`**, **[`run-intent-consumer.yml`](../../.github/workflows/run-intent-consumer.yml)** may **auto** validate → **`batch_generate`** → upload when intent **`upload`: true** (see `docs/spec/workflows.md` § run-intent-consumer). **Todo:** when planner **BLOCKED** (no intent JSON), auto consumer should **skip cleanly** — [`cohesion-readiness-review-todos.md`](cohesion-readiness-review-todos.md) **CR-7**. **Still optional / later:** CTR/impressions in fetch when API allows; richer personal-only tooling — still **separate files**, no blended correlate rows. **Audit:** **`scripts/audit_channel.py`** → `data/reports/audit-*-personal.md` (ledger join uses optional `channel` on `generations.json` + `workflow` inference; see `docs/spec/workflows.md` § analytics-personal).
