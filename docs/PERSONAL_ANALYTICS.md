...
2. **Parity (optional):** personal **`correlate` / `suggestions_personal.json`**, CTR/impressions in fetch when API allows — still **separate files**, no blended correlate rows. **Audit:** personal workflow runs **`scripts/audit_channel.py`** → `data/reports/audit-*-personal.md` (ledger join uses optional `channel` on `generations.json` + `workflow` inference; see `docs/spec/workflows.md` § analytics-personal).
